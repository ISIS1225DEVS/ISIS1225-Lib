...
@pytest.fixture
def lst():
    lst = lt.newList(datastructure='DOUBLE_LINKED',
                     key='book_id')
    return lst
...
@pytest.fixture
def lstbooks(books):
    lst = lt.newList(datastructure='DOUBLE_LINKED', key='book_id')
    for i in range(0, 7):
...
def test_addFirst(lst, books):
    ...
    assert lt.firstElement(lst) == books[1]
    assert lt.lastElement(lst) == books[1]
    assert lt.getElement(lst, 1) == books[1]
    lt.addFirst(lst, books[2])
    assert lt.size(lst) == 2
    book = lt.firstElement(lst)
    assert book == books[2]
    assert lt.lastElement(lst) == books[1]
    lt.addFirst(lst, books[3])
...
def test_removeFirst(lstbooks, books):
    assert lt.size(lstbooks) == 7
    assert lt.firstElement(lstbooks) == books[0]
    assert lt.lastElement(lstbooks) == books[6]
    assert lt.getElement(lstbooks, 1) == books[0]
    lt.removeFirst(lstbooks)
    assert lt.size(lstbooks) == 6
    book = lt.getElement(lstbooks, 1)
    assert book == books[1]
    assert lt.lastElement(lstbooks) == books[6]
...
def test_removeLast(lstbooks, books):
    ...
    assert lt.size(lstbooks) == 6
    assert lt.firstElement(lstbooks) == books[0]
    assert lt.lastElement(lstbooks) == books[5]
    assert lt.getElement(lstbooks, 1) == books[0]
...
def test_insertElement(lst, books):
    ...
    assert lt.size(lst) == 1
    assert lt.firstElement(lst) == books[0]
    assert lt.lastElement(lst) == books[0]
    lt.insertElement(lst, books[1], 1)
    assert lt.size(lst) == 2
    assert lt.firstElement(lst) == books[1]
    assert lt.lastElement(lst) == books[0]
    lt.insertElement(lst, books[2], 2)
    assert lt.size(lst) == 3
    book = lt.getElement(lst, 1)
    assert book == books[1]
    book = lt.getElement(lst, 3)
    assert book == books[0]
