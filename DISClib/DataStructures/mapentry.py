"""
Este ADT representa la registro de un Mapa no Ordenado (Entry), el cual es una estructura de datos para almacenar la información en parejas de llave-valor (key-value). La llave (key) es un identificador único para cada valor (value) almacenado en el Mapa no Ordenado (Map). Por su parte, el valor (value) puede ser cualquier tipo de dato.
